 ...
 */
const assertNotNull = (value, name) => {
    if (value === null) {
        if (name) {
...
if (!playCartoonButtonElement || !playWoodenButtonElement || !globalVolumeRangeElement || !displayVolumeRangeElement || !playingSoundsListElement || !playPenClickingElement)
    throw 'Elemento faltando no HTML';
...
class RangeInputComponent {
    constructor(label, name, defaulValue = 0, min, max, step, callbackonChange) {
        this.label = label;
        this.name = name;
...
class ListItemComponent {
    ...
    buildElements() {
        ...
        this.volumeInputComponent = new RangeInputComponent('Volume', 'volume', 100, 0, 100, 1, (value) => {
